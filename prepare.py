...
def process_mask(mask):
    ...
    for i_layer in range(convex_mask.shape[0]): # 对每一层进行处理
        ...
        mask1  = np.ascontiguousarray(mask[i_layer]) # 将一个内存不连续存储的数组转换为内存连续存储的数组，使得运行速度更快
        if np.sum(mask1)>0:
    ...
    dilatedMask = binary_dilation(convex_mask,structure=struct,iterations=10) # 膨胀运算
    return dilatedMask  
...
def savenpy_luna(id, annos, filelist, luna_segment, luna_data,savepath): # 转换成numpy 格式 在处理的过程中这里使用了多线程
    ...
    # 加载原图和掩码
    slice_im,slice_origin,slice_spacing,slice_isflip = load_itk_image(os.path.join(luna_data,name+'.mhd')) #加载原始数据
    mask_im,mask_origin,mask_spacing,mask_isflip = load_itk_image(os.path.join(luna_segment,name+'.mhd')) #加载相应的掩码
    ...
    # 这里乘以spacing相当于还原成原图
    box = box*np.expand_dims(mask_spacing,1)  /  np.expand_dims(resolution,1) #对边界即掩码的最小外部长方体应用新分辨率
    # np.floor   Return the floor of the input, element-wise.
    box = np.floor(box).astype('int')
    margin = 5
    newshape = np.round(np.array(mask_im.shape)*mask_spacing/resolution).astype('int') # 计算Mask的边界坐标
    ...
    if isClean:
        ...
        slice_im = slice_im*dilated_mask  +  pad_value*(1-dilated_mask).astype('uint8') # 掩码不覆盖的区域都填充为水
        bones = (slice_im*extra_mask)>bone_thresh   #extramask区域可能有骨头 210对应归一化后的骨头，凡是大于骨头的区域都填充为水
        slice_im[bones] = pad_value
        
        slice_im1,_ = resample(slice_im,slice_spacing,resolution,order=1)  #采用新分辨率
        slice_im2 = slice_im1[extendbox[0,0]:extendbox[0,1],    #将extendbox内数据取出作为最后结果
                    extendbox[1,0]:extendbox[1,1],
                    extendbox[2,0]:extendbox[2,1]]
        slice_im = slice_im2[np.newaxis,...]
        np.save(os.path.join(savepath, name+'_clean.npy'), slice_im) # 调整亮度 去除骨头和肺泡 以及其他区域
        np.save(os.path.join(savepath, name+'_spacing.npy'), slice_spacing) # 源文件 slice_spacing
        np.save(os.path.join(savepath, name+'_extendbox.npy'), extendbox) # 框出肺部的框框
    ...
    if islabel:
        ...
        if len(this_annos)>0:
            for c in this_annos: # 对于这一套CT 的每一个标注
                # position 坐标
                pos = worldToVoxelCoord(c[1:4][::-1],origin=slice_origin,spacing=slice_spacing) #将世界坐标转换为体素坐标
                if slice_isflip:
        ...
        if len(label)==0: #若没有结节则设为全0
            ...
            label2[:3] = label2[:3]*np.expand_dims(slice_spacing,1)/np.expand_dims(resolution,1) #对标签应用新的分辨率
